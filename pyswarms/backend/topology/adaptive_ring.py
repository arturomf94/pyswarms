...
"""
An Adaptive Ring Network Topology

This class implements a modified ring topology. In this topology,
the particles are connected with their k nearest neighbors if
they are feasible but can lose each connection with probability
pr if the are not feasible.
"""
...
class AdaptiveRing(Topology):
    def __init__(self, static=False):
        ...
        """
        super(AdaptiveRing, self).__init__(static)
    ...
    def compute_gbest(self, swarm, p, k, **kwargs):
        ...
        try:
            ...
            if k == 1:
                ...
                idx_min = ops.get_best_neighbors(swarm.pbest_cost[self.neighbor_idx])
                best_neighbor = ops.get_best_neighbors_ids(self.neighbor_idx, idx_min)
            # Obtain best position
            best_pos = ops.get_best_neighbors_positions(swarm.pbest_pos, best_neighbor)
    ...
    def compute_constrained_velocity(
        ...
        .. code-block :: python
            ...
            my_topology = AdaptiveRing(static=False)
        ...
        """
        return ops.compute_constrained_velocity(swarm, clamp, vh, bounds)
